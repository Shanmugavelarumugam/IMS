...
import { X, CheckCircle2, Box, IndianRupee, AlertTriangle, Layers, Settings } from 'lucide-react';

interface MetricsConfigModalProps {
  onClose: () => void;
  activeCardIds: string[];
  onChangeActiveCards: (id: string, checked: boolean) => void;
}
              ...
              return (
                ...
                >
                  ...
                  <input 
                    type="checkbox"
                    checked={isActive}
                    onChange={(e) => onChangeActiveCards(card.id, e.target.checked)}
                    style={{ display: 'none' }}
                  />
                </label>
              );
